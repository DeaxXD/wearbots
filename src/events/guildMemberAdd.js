...
module.exports = async member => {
    ...
    if (kurulus > 604800000) {
        ...
        let hosgeldinemd = new MessageEmbed()
        .setDescription(`
        ...
        Kayıt Olmak İçin İsim Yaş Vermen Yeterli

        Kayıt olduktan sonra <#1031253209395761282> Kanalını Okumayı Unutma
        
        `)
        .setColor('YELLOW')
        member.guild.channels.cache.get(config.channels.welcomechannel).send({embeds: [hosgeldinemd]});
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`${member} - <@&${config.registration.staff}>`)
    } else {
